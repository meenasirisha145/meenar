...
g2=ggplot(df1, aes(y = df1$Asales1, x=Acoy1)) + geom_violin(color='yellow', fill="blue")
g2
...
c1 = ggplot(data=df1, aes( x=Acoy1, fill=Aloc1)) + geom_bar(position = 'stack') + 
  labs(title='position=stack')
(c1) #stack on top
c2 = ggplot(data=df1, aes(x=Acoy1, fill=Aloc1)) + geom_bar(position = 'dodge') + 
  labs(title='position=dodge')
(c2) #side

c3 = ggplot(data=df1, aes(x=Acoy1, fill=Aloc1)) + geom_bar(position = 'fill') + 
...
ggplot(df1, aes(x=Aadv1, y=Asales1, color=Aloc1, shape=factor(Asalesman1))) +
  geom_point() + facet_grid(.~ Aloc1)
...
(r1a + r2 + r3)
#warning
ggplot(data=df1, aes(x=Aadv1, y=Asales1, color=Asalesman1)) + scale_color_manual
(values=c('red','blue','yellow')) + geom_point(size=2)

ggplot(data=df1, aes(x=Aadv1, y=Asales1, color=Asalesman1)) + scale_color_brewer(palette="Set1") +
  geom_point(size=3)
#preset colors
...
ggplot(data=df1, aes(x=Aadv1, y=Asales1, linetype=Asalesman1, shape=Acoy1)) + 
...
p2 <- ggplot(df1, aes(Aadv1, Aadv2)) +
  geom_point(data = df1[df1$Asales1 > 50,], aes(col=Asales1)) 
p2

p3 <- ggplot(df1, aes(Aadv1,Aadv2)) +
  geom_point(data = df1[df1$Asalesman1==1,],aes(col=Asalesman1)) 

p3
 
p4 <- ggplot(df1, aes(Aadv1,Aadv2)) +
  geom_point(data = df1[df1$Asales1>70,],aes(col=Asalesman1)) 
